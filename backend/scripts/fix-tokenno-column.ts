// Fix tokenNo column name in production database
...
async function fixTokenNoColumn() {
  console.log('🔧 Fixing tokenNo column name in production database...');
  ...
  try {
    ...
    // Step 2: Check for tokenNo vs tokenno
    ...
    // Step 3: Fix column name if needed
    if (!hasTokenNo && hasTokenNoLower) {
      console.log('🔧 Renaming tokenno to tokenNo...');
      
      // First, drop the existing tokenno column
      await prisma.$executeRaw`ALTER TABLE transactions DROP COLUMN tokenno`;
      console.log('✅ Dropped tokenno column');
      
      // Then add the tokenNo column with correct name
      await prisma.$executeRaw`ALTER TABLE transactions ADD COLUMN tokenNo INTEGER`;
      console.log('✅ Added tokenNo column');
      
    } else if (hasTokenNo) {
      console.log('✅ tokenNo column already exists with correct name');
    } else {
      console.log('⚠️ No tokenNo column found, adding it...');
      await prisma.$executeRaw`ALTER TABLE transactions ADD COLUMN tokenNo INTEGER`;
      console.log('✅ Added tokenNo column');
    }
    ...
    console.log('🎉 Production database tokenNo column fix completed!');
    
  } catch (error) {
    console.error('❌ Error fixing tokenNo column:', error.message);
    process.exit(1);
  }
}
